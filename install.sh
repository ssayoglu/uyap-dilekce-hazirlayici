...
# 2. Dosyaları doğrudan GitHub ZIP olarak indir (Xcode uyarısını %100 engeller)
...
# 3. İzinleri ayarla
chmod +x "$TARGET_DIR/server.py" "$TARGET_DIR/DilekceApp" "$TARGET_DIR/build_app.sh" 2>/dev/null || true

# 4. .app Paketini oluştur
echo -e "${YELLOW}🔨 macOS Uygulama Paketi oluşturuluyor...${NC}"
...
# 5. Uygulamalar ve Masaüstüne yerleştir
